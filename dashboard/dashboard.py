...
def create_day_df(df):
    """
    Fungsi untuk memuat dan menyiapkan data day_df dari file CSV.

    Args:
        day.csv (str): Path ke file CSV yang berisi data.

    Returns:
        pd.DataFrame: DataFrame yang telah diproses dan siap digunakan.
    """
    # Memuat data dari file CSV
    day_df = create_day_df(day.csv)
...
# Memastikan kolom datetime diurutkan
day_df.sort_values(by="dteday", inplace=True)
...
# Pivot table
pivot_table = day_df.groupby(by="month").agg({
    "instant": "nunique",
